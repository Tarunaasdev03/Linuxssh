...
if st.button("🔍 Check Eligibility"):
    ...
    share_link = f"https://wa.me/?text={urllib.parse.quote_plus(share_text)}"  # Fixed URL encoding
...
# Validate user input
if user_funds < emi:
    st.warning(f"⚠ Your current funds ({user_funds:,.0f}) are insufficient for EMI ({emi:,.0f}). Consider angel money providers.")
    
    # Define angel money interest rate
    angel_interest = 1.5  # Example interest rate
    repayment_amount = emi + (emi * (angel_interest / 100))
    
    st.info(f"🕊 Angel Money EMI Support: Your new repayment amount will be ₹{repayment_amount:,.0f} per month.")
    ...
    share_link_loan = f"https://wa.me/?text={urllib.parse.quote_plus(share_text_loan)}"  # Fixed URL encoding
